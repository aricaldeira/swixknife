...
CALC = SezimalCalculator(locale=_get_android_locale())

RECURRING_SEPARATOR_ALLOWED = re.compile('^[0-9]{1,}$|\.[0-9][0-9_]{0,}$')


class SKButton(MDFlatButton):
    pass

class SKLongPressButton(SKButton):
...
class SezimalGrid(MDGridLayout):
    ...
    def update_display(self):
        ...
        self.recurring_separator.disabled = not RECURRING_SEPARATOR_ALLOWED.search(CALC.expression)
...
class DecimalGrid(MDGridLayout):
    def do_layout(self, *args, **kwargs):
        super().do_layout()
        self.fraction_separator.text = CALC.locale.SEZIMAL_SEPARATOR
        self.recurring_separator.text = CALC.locale.SEZIMAL_SEPARATOR + CALC.locale.SEZIMAL_SEPARATOR
        self.update_display()
