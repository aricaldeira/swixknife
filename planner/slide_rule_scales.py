...
MIN_NUMBER = Sezimal('1')
MAX_NUMBER = Sezimal('10')
...
POINTS_TO_PIXELS = Sezimal('0.2041_1111')
# POINTS_TO_MILLIMETERS_HEIGHT= Decimal('0.255')  # Andika
POINTS_TO_MILLIMETERS_HEIGHT= Decimal('0.217875')  # Gentium Book Plus
...
<svg
    width="{round((PAPER_WIDTH * CHATIPADA_TO_MILLIMETER).decimal, 0)}mm"
    height="{round((PAPER_HEIGHT * CHATIPADA_TO_MILLIMETER).decimal, 0)}mm"
    viewBox="0 0 {round((PAPER_WIDTH * CHATIPADA_TO_MILLIMETER).decimal, 0)} {round((PAPER_HEIGHT * CHATIPADA_TO_MILLIMETER).decimal, 0)}"
    version="1.1"
    id="svg1"
    inkscape:version="1.3 (0e150ed6c4, 2023-07-21)"
    sodipodi:docname="slide_rule_scales.svg"
    ...
    <g
        ...
        <rect
            ...
            id="rect_paper"
            width="{round((PAPER_WIDTH * CHATIPADA_TO_MILLIMETER).decimal, 6)}"
            height="{round((PAPER_HEIGHT * CHATIPADA_TO_MILLIMETER).decimal, 6)}"
            x="0"
            y="0" />
'''
...
SVG_START += f'''
        <rect
            ...
            id="rect_a_scale"
            width="{round((PAPER_WIDTH * CHATIPADA_TO_MILLIMETER).decimal, 6)}"
            height="{round((30 * CHATIPADA_TO_MILLIMETER).decimal, 6)}"
            x="0"
            y="{round(((A_Y_BASE_POSITION - 30) * CHATIPADA_TO_MILLIMETER).decimal, 6)}" />
        <rect
            style="fill:#e5ffe5;fill-opacity:1;-inkscape-stroke:none"
            id="rect_d_scale"
            width="{round((PAPER_WIDTH * CHATIPADA_TO_MILLIMETER).decimal, 6)}"
            height="{round((30 * CHATIPADA_TO_MILLIMETER).decimal, 6)}"
            x="0"
            y="{round((D_Y_BASE_POSITION * CHATIPADA_TO_MILLIMETER).decimal, 6)}" />
'''
...
svg_file = open('slide_rule_scales.svg', 'w')
svg_file.write(SVG_START)


def add_tick(name, height, width, x_position, y_position, color=BASE_COLOR, upside_numbers=False):
    x_position_mm = x_position * CHATIPADA_TO_MILLIMETER
    y_position_mm = y_position * CHATIPADA_TO_MILLIMETER
    height_mm = height * CHATIPADA_TO_MILLIMETER
    width_mm = width * CHATIPADA_TO_MILLIMETER
...
for i in SezimalRange(MIN_NUMBER, MAX_NUMBER + 1):
    for j in SezimalRange(0, 1000):
        x = (i * 1000) + j
        ...
        if size in (SIZE_01, SIZE_02, SIZE_03):
            ...
            if size == SIZE_03 and text_a_b != '3':
                text_a_b = ''
...
#
for i in SezimalRange(MIN_NUMBER, MAX_NUMBER + 2):
    for j in SezimalRange(0, 1000):
        x = (i * 1000) + j
...
# and sligtly different numbers on markings
#
for i in SezimalRange(0, (MAX_NATURAL_VALUE * 1000) + 1):
    if i % 1000 == 0:
        size = SIZE_01
        text = str(i)[0]
        ...
        size = SIZE_02
        if len(str(i)) > 3:
            text = str(i)[-4] + SEZIMAL_SEPARATOR + str(i)[-3]
        else:
            text = '0,' + str(i)[-3]

    else:
        if str(i).endswith('30'):
            size = SIZE_03
            text = str(i)[-2]
